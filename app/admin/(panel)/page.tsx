        ...
        <div className="bg-navy-dark px-8 py-6">
          ...
          <div className="relative flex items-end justify-between">
            ...
            <Link
              ...
              className="flex items-center gap-2 bg-gold text-navy-dark text-sm font-semibold px-4 py-2.5 rounded-xl hover:bg-gold-light transition-all duration-200 active:scale-95 shrink-0"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {stats.map(({ label, value, icon: Icon, href, accent, bar }) => (
            <Link
              key={label}
              href={href}
              className="group bg-white rounded-2xl border border-gray-100 p-5 hover:border-gray-200 hover:shadow-md transition-all duration-200 flex flex-col gap-3"
            >
            ...
            {services.length === 0 ? (
              ...
              <div className="divide-y divide-gray-50">
                {services.slice(0, 6).map((service) => (
                  <div
                    key={service.id}
                    className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50/60 transition-colors duration-150 group"
                  >
                    {/* Agency badge */}
