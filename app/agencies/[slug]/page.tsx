...
export default async function AgencyDetailPage({ params }: Props) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Agency header */}
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-white/10 overflow-hidden mb-8 animate-fade-in-up">
        {/* Gold gradient top strip */}
        <div className="h-[3px] bg-gradient-to-r from-navy via-gold to-navy" />

        <div className="p-6">
          <div className="flex items-start gap-5">
            <div className="w-16 h-16 rounded-2xl bg-navy/5 dark:bg-white/5 flex items-center justify-center shrink-0 overflow-hidden ring-2 ring-navy/10 dark:ring-white/10">
              {agency.logoUrl ? (
                <Image
                  src={agency.logoUrl}
                  alt={agency.acronym}
                ...
                <span className="font-display font-extrabold text-navy dark:text-gray-300 text-sm text-center leading-tight">
                  {agency.acronym}
                </span>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <h1 className="font-display font-bold text-2xl text-navy dark:text-white tracking-tight">
                {agency.name}
              </h1>
      ...
      <div className="animate-fade-in-up animation-delay-100">
        <h2 className="font-display font-semibold text-lg text-navy dark:text-white mb-5">
          Services
          <span className="text-gold font-normal ml-1.5">({agency.services?.length ?? 0})</span>
        </h2>
        {agency.services && agency.services.length > 0 ? (
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-white/10 py-14 text-center">
            <p className="text-sm text-gray-400 dark:text-gray-500">No services available for this agency yet.</p>
          </div>
        )}
      </div>
    </div>
