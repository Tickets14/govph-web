...
export function AgencyActions({ id, acronym }: AgencyActionsProps) {
  ...
  const handleDelete = async () => {
    ...
    const res = await adminClientFetch(`/api/admin/agencies/${id}`, { method: 'DELETE' });
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">Delete agency</p>
                  <p className="text-xs text-gray-400 mt-1 leading-relaxed">
                    Are you sure you want to delete{' '}
                    <span className="font-medium text-gray-600 dark:text-gray-300">{acronym}</span>? This action cannot
                    be undone.
                  </p>
                </div>
              </div>
