...
export class AppError extends Error {
  ...
  constructor(message: string, code: ErrorCode, statusCode = 400, context?: Record<string, unknown>) {
    super(message);
    this.name = 'AppError';
    this.code = code;
  ...
  static notFoundBySlug(resource: string, slug: string): AppError {
    return new AppError(`${resource} with slug "${slug}" not found`, 'NOT_FOUND', 404, { resource, slug });
  }

  static alreadyExists(resource: string, field: string, value: string): AppError {
    return new AppError(`${resource} with ${field} "${value}" already exists`, 'ALREADY_EXISTS', 409, {
      resource,
      field,
      value,
    });
